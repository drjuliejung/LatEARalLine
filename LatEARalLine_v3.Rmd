...
range(Hat$Tadpole_length, na.rm=TRUE)
range(Hat$Final_Stages, na.rm=TRUE)
```
...
# first align the top-row plot (Figure4a) with the left-most plots of the next two lines

# top row
plots <- align_plots(Figure4a, Figure4c, plot.LL.GLM, align = 'v', axis = 'l')
# then build the top row
top_row <- plot_grid(plots[[1]], Figure4b, labels = c('A', 'B'), label_size = 12)

# then combine with the bottom row for final plot
Figure4v8 <- plot_grid(top_row, plots[[2]], plots[[3]], labels = c(' ', 'C', 'D'), label_size = 12, ncol = 1)

ggsave("~/Desktop/Figure4v8.pdf",
 plot = Figure4v8, # or give ggplot object name as in myPlot,
 width = 15, height = 20,
 units = "cm", # other options c("in", "cm", "mm"),
 dpi = 300, 
 useDingbats=FALSE)
...
neuromast_totals <- 
  Diasp.df %>%
  group_by(Final_Stages) %>%
  summarize("Total" = mean((JJ_total + AC_total)/2), 
            "TotalSEM" = se((JJ_total + AC_total)/2)/sqrt(n())
            )
kable(neuromast_totals, digits=1)
```
...
neuromast_lines_long <- neuromast_lines %>% 
  pivot_longer(-c(Final_Stages,n), names_to="line", values_to = "meanNM")

neuromast_lines_se_long <- neuromast_lines_se %>% 
  pivot_longer(-c(Final_Stages,n), names_to="line", values_to = "seNM")

neuromast_lines_long$seNM <- neuromast_lines_se_long$seNM
kable(neuromast_lines_long, digits=1)
```
...
plot.nm.lines <- ggplot(neuromast_lines_long, aes(x=Final_Stages, y=meanNM, color=line)) +
  geom_errorbar(aes(ymin=meanNM-seNM, ymax=meanNM+seNM), width=0.05, size=1.5) +
...
```{r}
Diasp.df$JJAC_naris_totalsides <- (Diasp.df$JJ_naris_totalsides + Diasp.df$AC_naris_totalsides)/2
Diasp.df$JJAC_ventral_totalsides <- (Diasp.df$JJ_ventral_totalsides + Diasp.df$AC_naris_totalsides)/2
Diasp.df$JJAC_oral_totalsides <- (Diasp.df$JJ_oral_totalsides + Diasp.df$AC_oral_totalsides)/2
Diasp.df$JJAC_infraorbital_totalsides <- (Diasp.df$JJ_infraorbital_totalsides + Diasp.df$AC_infraorbital_totalsides)/2
Diasp.df$JJAC_supraorbital_frontal_totalsides <- (Diasp.df$JJ_supraorbital_frontal_totalsides + Diasp.df$AC_supraorbital_frontal_totalsides)/2
Diasp.df$JJAC_supraorbital_1side <- (Diasp.df$JJ_supraorbital_1side + Diasp.df$AC_supraorbital_1side)/2
Diasp.df$JJAC_middle_1side <- (Diasp.df$JJ_middle_1side + Diasp.df$AC_middle_1side)/2
Diasp.df$JJAC_dorsal_totalsides <- (Diasp.df$JJ_dorsal_totalsides + Diasp.df$AC_dorsal_totalsides)/2

hist(Diasp.df$JJ_naris_totalsides)
temp1<-glmer(JJAC_naris_totalsides ~ 1 + (1|Clutch), family=Gamma(inverse), data=Diasp.df)
temp2<-glmer(JJAC_naris_totalsides ~ Final_Stages + (1|Clutch), family=Gamma(inverse), data=Diasp.df)

anova(temp1, temp2) #line effect
anova(temp2, temp3) #stage effect
```
...
all_data$Stimulus_start_age <- all_data$Age_days + all_data$Stimulus_h/24 + all_data$Stimulus_m/(24*60) + all_data$Stimulus_s/(24*60*60) # Stimulus_start_age is in days
...
# #How many clutches is this from?
# older_group_jiggling<-rbind(middle_jiggling, older_jiggling)
# 
# temp <-
#   older_group_jiggling %>%
#   filter(!is.na(Sample_name)) %>%
#   group_by(Clutch) %>%
#   summarize(num_tested = n(),
...
#   summarize(num_tested = n(),
#             mean = mean(meanVOR),
#             SE = sd(meanVOR)/sqrt(n())
#             )
# kable(temp2,digits=4)
...
How many individuals lack both vestibular function and lateral line function & hatched?
```{r}
GT_Hat_lack<-subset(younger_jiggling_low, younger_jiggling_low$Treatment=="G" & younger_jiggling_low$Response==1)
...
```{r}
library(gamlss)
glmm0 <- gamlss(Latency_min ~ 1 + (1|Clutch), family=WEI3, method=CG(), data=jiggling.df)
glmm1 <- glmer(Latency_min ~ Treatment + (1|Clutch), family=Gamma(inverse), data=jiggling.df)
glmm2 <- glmer(Latency_min ~ Age_groups_four + (1|Clutch), family=Gamma(inverse), data=jiggling.df)
glmm3 <- glmer(Latency_min ~ Treatment + Age_groups_four + (1|Clutch), family=Gamma(inverse), data=jiggling.df)
glmm4 <- glmer(Latency_min ~ Treatment * Age_groups_four + (1|Clutch), family=Gamma(inverse), data=jiggling.df)
...
str(all_ages_by_clutch)
```


```{r}
NT.jig<-subset(all_ages_by_clutch, all_ages_by_clutch$Treatment == "NT")
GT.jig<-subset(all_ages_by_clutch, all_ages_by_clutch$Treatment == "G")

plot.jiggle.hatch.v2 <- all_ages_by_clutch %>%
  ggplot(aes(x=as.factor(AgeD), y=propH, color=Treatment))+ 
    geom_boxplot(position="dodge", 
             width=0.4, outlier.shape = NA)+ 
    geom_jitter(data=NT.jig, aes(x=as.factor(AgeD), y=propH), colour="black", pch=16, size=2, position=position_jitter(width=0.1)) +
  geom_jitter(data=GT.jig, aes(x=as.factor(AgeD), y=propH), colour="red", pch=16, size=2,  position=position_jitter(width=0.1)) +
...
str(latency_all_ages_by_clutch)
...
plot.jiggle.latency.v2 <- latency_all_ages_by_clutch %>%
  ggplot(aes(x=as.factor(AgeD), y=ltoh, color=Treatment))+ 
    geom_boxplot(position="dodge", 
             width=0.4, outlier.shape = NA)+ 
    geom_jitter(data=latency_NT.jig, aes(x=as.factor(AgeD), y=ltoh), colour="black", pch=16, size=2, position=position_jitter(width=0.1)) +
  geom_jitter(data=latency_GT.jig, aes(x=as.factor(AgeD), y=ltoh), colour="red", pch=16, size=2,  position=position_jitter(width=0.1)) +
  #facet_wrap(facets=.~AgeD)+ 
  #scale_x_continuous(limits=c(4.1,5.5),
  #                   breaks=c(4.2, 4.7, 5.4),
  #                   labels=c("4.2", "4.7", "5.4"))+
...
combine plots 
```{r}
ggsave("~/Desktop/ForKaren.pdf", 
 plot = plot.jiggle.hatch.v2, # or give ggplot object name as in myPlot,
 width = 10, height = 10, 
 units = "cm", # other options c("in", "cm", "mm"), 
 dpi = 300, 
 useDingbats=FALSE)
...
mean(test$TestEggs) #mean
sd(test$TestEggs)/sqrt(length(test$TestEggs)) #se
...
```{r}
quantile(test$PropH)
quantile(test$PropH, probs=c(0.05, 0.25, 0.5, 0.75, 0.95))
t.test(test$PropH)$"conf.int"
```
...
playbacks_by_treatment<-
  ...
  summarize(count = n(),
            ...
            CI.lower = first(t.test(PropH)$"conf.int"), 
            CI.higher = last(t.test(PropH)$"conf.int") 
            )
...
How often did we have multiple trays from a clutch within a treatment?
...
kable(tray_table,digits=4)

tab_df(tray_table,
       title = "Tray Table",
       file="Desktop\tray_table.doc"
       )
...
#fit_sigmoidal <- fitconds(playback3, group="Treatment", varnames=c(K = "PropH", WP = "latencies_sec"), model="sigmoidal", Kmax=1, nboot=1000)
#plot(fit_sigmoidal, onepanel=TRUE)
...
sum(Paired_data_similar_stages$num_tested)
nrow(Paired_data_similar_stages)/2 #47 clutches
```
For Fig 1 - 
```{r}
# str(all_data)
# #find one pair of sibs - one T and one NT, same stage and age. 
# ForFig1 <-
#   all_data %>%
#   filter(!is.na(Sample_name), !is.na(Stimulus_start_age), !is.na(LL1start)) %>%
#   group_by(Clutch, Treatment) %>%
#   summarize(num_tested = n(),
#             mean_amp = mean(Mean_VOR, na.rm=T),
#             mean_stage = mean(Combined_stage, na.rm=T),
#             mean_age = mean(Stimulus_start_age, na.rm=T)
#             )%>%
#   mutate(count=n()) %>%
#   filter(count==2) %>%
#   mutate(Difference_stage = mean_stage - lag(mean_stage)) %>% 
#   mutate(Difference_age = mean_age - lag(mean_age)) %>% 
#   #also filter out the pairs where the mean stage between treatments is really different (>1)
#   mutate(Difference_new_stage = ave(Difference_stage, Clutch, FUN=function(x)unique(x[!is.na(x)]))) %>%
#  mutate(Difference_new_age = ave(Difference_age, Clutch, FUN=function(x)unique(x[!is.na(x)]))) %>%
#   #fill in lag so that both indivs have difference value.
#   filter(Difference_new_stage == 0, Difference_new_age < 0.05) 
# kable(ForFig1,digits=4)
# 
# chooseme<- all_data%>%
#   subset(Clutch==133) %>%
#   filter(!is.na(LL1start))
...
How many clutches are there where I have VOR measurements from pairs (NT and G) near the same stage
vs. how many clutches where addition data with no pairs (near the same stage). 

```{r}
Paired_data_similar_stages_experiment <-
  all_data %>%
  filter(!is.na(Sample_name)) %>%
  group_by(Experiment, Clutch, Treatment) %>%
  summarize(num_tested = n(),
            mean_amp = mean(Mean_VOR, na.rm=T),
            mean_stage = mean(Combined_stage, na.rm=T)
            )%>%
...
kable(Paired_data_similar_stages_experiment,digits=4)

nrow(Paired_data_similar_stages_experiment)/2 #56 clutches total
nrow(subset(Paired_data_similar_stages_experiment,Paired_data_similar_stages_experiment$Experiment =="Individual"))/2 #24 clutches for jiggling
nrow(subset(Paired_data_similar_stages_experiment,Paired_data_similar_stages_experiment$Experiment =="Tray"))/2 #32 clutches for playback


#how many clutches in 2 older 
```

how many are unpaired?
```{r}
Single_data_similar_stages_experiment <-
  all_data %>%
  filter(!is.na(Sample_name)) %>%
  group_by(Experiment, Clutch, Treatment) %>%
  summarize(num_tested = n(),
...
kable(Single_data_similar_stages_experiment,digits=4)

nrow(Single_data_similar_stages_experiment) #43 single clutches total
nrow(subset(Single_data_similar_stages_experiment,Single_data_similar_stages_experiment$Experiment =="Individual")) #29 clutches for jiggling
nrow(subset(Single_data_similar_stages_experiment,Single_data_similar_stages_experiment$Experiment =="Tray")) #14 clutches for playback
```

```{r}
# NT_paired_exp<-subset(Paired_data_similar_stages_experiment, Paired_data_similar_stages_experiment$Treatment=="NT")
# GT_paired_exp<-subset(Paired_data_similar_stages_experiment, Paired_data_similar_stages_experiment$Treatment=="G")
# 
# Figure_VOR<-ggpaired(data=Paired_data_similar_stages_experiment, x="Treatment", y="mean_amp", color="Treatment", line.color = "gray", line.size = 0.4, palette = "npg") + 
...
single<-subset(Paired_data_similar_stages_experiment, Paired_data_similar_stages_experiment$count==1) #41 clutches
double<-subset(Paired_data_similar_stages_experiment, Paired_data_similar_stages_experiment$count==2) #22 clutches
```
...
```{r}
Figure_VOR<-ggplot(data=Paired_data_similar_stages, aes(x=Treatment, y=mean_amp, color=Treatment)) + 
  geom_boxplot(alpha = 1, size = 1, width = 0.15, outlier.shape=NA) +
  geom_jitter(data=NT_paired, aes(x=Treatment, y=mean_amp), colour="black", pch=16, size=2, position=position_jitter(width=0.05)) +
  geom_jitter(data=GT_paired, aes(x=Treatment, y=mean_amp), colour="red", pch=16, size=2,  position=position_jitter(width=0.05)) +
  scale_y_continuous(limits=c(0, 60),
                     breaks=c(0, 20, 40, 60),
                     labels=c("0", "20", "40", "60"))+
  scale_x_discrete(labels=c("Gentamicin", "Control")) + 
...
We found that MCH in response to jiggling cues occurs before vestibular function at a rate of ~11% in non-treated individuals 
...
```{r}
seven <- subset(tactile, VOR.amplitude..mean.sine.fit..excluding.non.VOR.eye.movements. == 0 & Hatching.response..0...no.hatch..1...hatched. == 1) #7 hatched with NO VOR

others <- subset(tactile, VOR.amplitude..mean.sine.fit..excluding.non.VOR.eye.movements. > 0 & Hatching.response..0...no.hatch..1...hatched. ==1) #54 hatched with VOR

hatched <- subset(tactile, Hatching.response..0...no.hatch..1...hatched. ==1) #61 hatched total

7/(7+54) #11% of those hatched had VOR. 
```
...
```{r}
LT10 <- subset(tactile, VOR.amplitude..mean.sine.fit..excluding.non.VOR.eye.movements. < 10) #51 low VOR
LT10_hat <- subset(LT10, Hatching.response..0...no.hatch..1...hatched.   == "1") #12 hatch with low VOR
LT10_nohat <- subset(LT10, Hatching.response..0...no.hatch..1...hatched.   == "0") #39 NH with low VOR
#propH = 12/51 = 0.2353
12/()

LT1 <- subset(tactile, VOR.amplitude..mean.sine.fit..excluding.non.VOR.eye.movements. < 1) #31 really low VOR
LT1_hat <- subset(LT1, Hatching.response..0...no.hatch..1...hatched.   == "1") #7 hatched with really low VOR
LT1_nohat <- subset(LT1, Hatching.response..0...no.hatch..1...hatched.   == "0") #24 NH with really low VOR
#propH = 7/31 = 0.2258
```
...
# 6 part fig
```{r}
FigureResults <- plot_grid(Fig5_inset_boxplot, figure.jiggle.hatch, figure.jiggle.latency, labels = "AUTO", ncol = 3, align = 'v', axis = "lrtb", rel_heights = c(1.1, 1.05, 1.2))



ggsave("~/Desktop/FigureResults_v1.pdf", 
 plot = FigureResults, # or give ggplot object name as in myPlot,
 width = 10, height = 20, 
 units = "cm", # other options c("in", "cm", "mm"), 
 dpi = 300, 
 useDingbats=FALSE)
...
If i restricted age from 5.3997 d to 5.5569 d  ~ or ~ if we restrict stage from 30 to 32 we would get some overlap. 
would there be enough sample size to conduct an analysis on latency? 
would the latency analysis change? 

Make a new dataframe "fived" including oldest jiggling age and all playbacks:
```{r}
fived <- subset(all_data, all_data$Experiment=="Tray" | all_data$Age_groups_three=="old")
```

```{r}
#let's take all older jiggled individuals that hatched and where we know their latencies to hatch

fived_latencies <- rbind(older_jiggling)
  
Latency_older_by_clutch #latencies by clutch
...
```{r}
str(test)
#want latencies by clutch of playbacks too 
...
#Latency_playback_by_clutch$AgeD <- 5.--
kable(Latency_playback_by_clutch,digits=4)
```

What's the range of overlap in age?
```{r}
...
What's the range of overlap in stage?
```{r}


```

Restrict age between 5.3997 and 5.5569 d (inclusive)
```{r}
fived_restAGE <- subset(fived, fived$Stimulus_start_age>=5.3997 & fived$Stimulus_start_age<=5.5569)
fived_restAGE_jigg <- subset(fived_restAGE, fived_restAGE$Experiment=="Individual")
fived_restAGE_play <- subset(fived_restAGE, fived_restAGE$Experiment=="Tray")
```

Restrict stage between 30 and 32 (inclusive)
```{r}
fived_restSTAGE <- subset(fived, fived$Combined_stage>=30 & fived$Combined_stage<=32)
fived_restSTAGE_jigg <- subset(fived_restSTAGE, fived_restSTAGE$Experiment=="Individual")
fived_restSTAGE_play <- subset(fived_restSTAGE, fived_restSTAGE$Experiment=="Tray")
```

Of the restricted stages, do we have enough latency data?
```{r}
fived_restSTAGE_jigg_ltoh <- subset(fived_restSTAGE_jigg, fived_restSTAGE_jigg$Latency_min>0)
nrow(fived_restSTAGE_jigg_ltoh) #31 individuals

str(fived_restSTAGE_play)
fived_restSTAGE_play_ltoh <- subset(fived_restSTAGE_play, fived_restSTAGE_play$Latency_min>0)
nrow(fived_restSTAGE_play_ltoh)
```